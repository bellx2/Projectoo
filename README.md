...
- **チーム** — チームの作成・編集・削除。メンバーは複数チームに所属可能。週報のチーム週報、課題一覧・ガントチャートのチーム絞り込みに連動
...
- 週報・変更履歴の週境界はサーバーのローカル時刻で判定します。海外リージョン (UTC など) にデプロイする場合は環境変数 `TZ=Asia/Tokyo` を設定してください。
- セッション Cookie の署名鍵はデフォルト値が入っています。公開環境では `SESSION_SECRET` を設定してください。
